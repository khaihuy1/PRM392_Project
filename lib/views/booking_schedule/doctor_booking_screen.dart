...
class _BookingScreenState extends State<BookingScreen> {
  ...
  Future<void> _loadAllData() async {
    try {
      ...
      // Chạy song song các task để tiết kiệm thời gian
      final results = await Future.wait([
        _doctorRepo.getDoctorDetail(widget.doctorId),
        _scheduleRepo.getSchedulesByDoctor(widget.doctorId),
        if (_currentUserId != null) _patientRepo.getProfilesByUser(_currentUserId!) else Future.value([]),
      ]);

      setState(() {
        _doctorDetail = results[0] as Map<String, dynamic>?;
        _schedules = results[1] as List<Schedule>;
        _profiles = results[2] as List<PatientProfile>;
        _isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> _loadSlots(int scheduleId) async {
    ...
    setState(() {
      ...
      _selectedSlot = null; // Reset khi đổi ngày
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_doctorDetail == null) return const Scaffold(body: Center(child: Text("Không tìm thấy bác sĩ")));

    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        title: const Text("Đặt lịch khám", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(_doctorDetail!['full_name'], style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                Text(_doctorDetail!['specialty_name'], style: const TextStyle(color: Colors.grey)),
                Text("📍 ${_doctorDetail!['clinic_name']}", style: const TextStyle(color: Colors.blue, fontSize: 12)),
                Text("💰 ${(_doctorDetail!['price'] ?? 0)} VNĐ", style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildScheduleList() {
    if (_schedules.isEmpty) return const Text("Bác sĩ chưa có lịch khám sắp tới.", style: TextStyle(color: Colors.grey));
    return SizedBox(
      height: 50,
  ...
  Widget _buildSlotGrid() {
    if (_selectedSchedule == null) return const Text("Vui lòng chọn ngày trước", style: TextStyle(color: Colors.grey, fontSize: 13));
    if (_availableSlots.isEmpty) return const Text("Ngày này đã hết lịch trống");
    return Wrap(
  ...
  Widget _buildPatientSelector() {
    return Column(
      children: [
        ..._profiles.map((profile) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  CircleAvatar(backgroundColor: Colors.blue.shade100, child: Text(profile.fullName[0])),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(profile.fullName, style: const TextStyle(fontWeight: FontWeight.bold)),
                        Text(profile.relationship, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                      ],
                    ),
                  ),
        ...
        TextButton.icon(
          onPressed: () async {
            final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => AddPatientScreen(userId: _currentUserId!)));
            if (result == true) _loadAllData(); // Reload list
          },
  ...
  Widget _buildConfirmButton() {
    bool canBook = _selectedSlot != null && _selectedProfile != null && !_isBooking;
    return Container(
  ...
  Future<void> _handleBooking() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lỗi: Khung giờ này vừa có người đặt!")));
    }
  }
  ...
  void _showSuccessDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Đặt lịch thành công!", textAlign: TextAlign.center),
        actions: [
          TextButton(
            onPressed: () => Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => MainScreen(userId: _currentUserId!)), (r) => false),
